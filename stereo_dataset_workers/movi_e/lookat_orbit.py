...
if FLAGS.camera == "fixed_random":
  ...
  center_position = kb.sample_point_in_half_sphere_shell(inner_radius=7., outer_radius=9., offset=0.1)
  left_cam, right_cam = create_rectified_stereo_pair(center_position=center_position, 
  ...
  # Create initial stereo pair
  outs = create_rectified_stereo_pair(baseline=FLAGS.baseline, 
                                      sensor_width=FLAGS.sensor_width, 
                                      focal_length=FLAGS.focal_length)
  left_cam = outs['left_camera']
  right_cam = outs['right_camera']
  scene.add([left_cam, right_cam])
  ...
  for frame in range(FLAGS.frame_start - 1, FLAGS.frame_end + 2):
    ...
    new_center_pos = (interp * np.array(camera_start) + (1 - interp) * np.array(camera_end))
    
    if is_panning:
      # for panning, we interpolate the lookat target as well
      target = interp * np.array(lookat_start) + (1 - interp) * np.array(lookat_end)
    else:
      # otherwise, we always look at the origin
      target = [0, 0, 0]  # look at origin
    ...
    left_cam.position = left_pos
    left_cam.quaternion = quat

    right_cam.position = right_pos
    right_cam.quaternion = quat

    # Sanity checks
    assert np.allclose(left_cam.quaternion, right_cam.quaternion), "Stereo cameras must have the same orientation!"
    assert np.allclose(
        np.linalg.norm(np.array(left_cam.position) - np.array(right_cam.position)),
        FLAGS.baseline,
    ), "Stereo baseline distance constraint violated!"

    # Broadcast change in camera pose to the renderer
    left_cam.keyframe_insert("position", frame)
    right_cam.keyframe_insert("position", frame)

    left_cam.keyframe_insert("quaternion", frame)
    right_cam.keyframe_insert("quaternion", frame)
...
for camera in [left_cam, right_cam]:
  logging.info(f"  Rendering from camera '{camera.name}'")
  scene.camera = camera
  ...
  logging.info("Storing rendered images to: %s", camera_output_dir)
  for key in data_stack.keys():
    (camera_output_dir / key).mkdir(parents=True, exist_ok=True)
  ...
  kb.write_json(filename=camera_output_dir / f"metadata_{camera.name}.json", data={
      "flags": vars(FLAGS),
      "metadata": kb.get_scene_metadata(scene),
      "camera": kb.get_camera_info(scene.camera),
      "instances": kb.get_instance_info(scene, visible_foreground_assets),
  })
  kb.write_json(filename=camera_output_dir / f"events_{camera.name}.json", data={
...
time_taken_mins = int((end - start) / 60.0)
logging.info(f"------------------- Total time taken: {time_taken_mins} minutes. --------------------")